...
class SuratMasuk extends CI_Controller {
	...
	public function index(){
		...
		if ( $level == 'admin' || $level == 'kapolsek') {
			$data['suratMasuk'] = $this->m_suratMasuk->get_all($date1,$date2);
		}else{
			$bagian = $this->session->userdata('id_bagian');
			$data['suratMasuk'] = $this->m_suratMasuk->get_all_bagian($bagian,$date1,$date2);
		}
	...
	public function tindaklanjut($id)
    {
        ...
        if ($row) {
			...
			$this->m_suratMasuk->update_disposisi_bag($id, $data);
			$error = $this->db->error();
	...
	public function printberkas(){
		...
		$this->load->view('suratMasuk/v_printberkas',$data);
